...
var getTrianglePeaks = function getTrianglePeaks(scale, sides) {
  var peakX = parseFloat((scale * sides[1] * Math.sin(Math.PI / 2 - Math.acos((Math.pow(sides[0], 2) + Math.pow(sides[1], 2) - Math.pow(sides[2], 2)) / (2 * sides[0] * sides[1])))).toFixed(1));
  var peakY = parseFloat((scale * Math.sqrt(Math.pow(sides[1], 2) - Math.pow(peakX / scale, 2))).toFixed(1));
  return {
    peakX: peakX,
    peakY: peakY
  };
